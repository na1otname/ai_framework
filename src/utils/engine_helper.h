...
{
    LOG_INFO("  index=%d, name=%s, n_dims=%d, dims=[%d, %d, %d, %d], n_elems=%d, size=%d, fmt=%s, type=%s, qnt_type=%s, "
             "zp=%d, scale=%f",
...
static rknn_tensor_format rknn_layout_convert(tensor_layout_e fmt)
{
    ...
    default:
        LOG_ERROR("unsupported nn layout: %d\n", fmt);
        // exit program
        exit(1);
    }
}
...
static rknn_tensor_type rknn_type_convert(tensor_datatype_e type)
{
    ...
    default:
        LOG_ERROR("unsupported nn type: %d\n", type);
        // exit program
        exit(1);
    }
}
...
static void rknn_output_to_tensor_data(const rknn_output &output, tensor_data_s &data)
{
    ...
    LOG_DEBUG("output size: %d", output.size);
    LOG_DEBUG("output want_float: %d", output.want_float);
